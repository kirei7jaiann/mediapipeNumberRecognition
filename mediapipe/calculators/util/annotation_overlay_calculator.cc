...
::mediapipe::Status AnnotationOverlayCalculator::GetContract(
  ...
  RET_CHECK(cc->Inputs().HasTag(assignmentPresentationTimeTag));
  cc->Inputs().Tag(assignmentPresentationTimeTag).Set<std::string>();

  RET_CHECK(cc->Inputs().HasTag(assignmentPresentationQuestionTag));
  cc->Inputs().Tag(assignmentPresentationQuestionTag).Set<std::string>();

  RET_CHECK(cc->Inputs().HasTag(assignmentPresentationAnswerTag));
  cc->Inputs().Tag(assignmentPresentationAnswerTag).Set<std::string>();

  RET_CHECK(cc->Inputs().HasTag(assignmentPresentationIsCorrectTag));
  cc->Inputs().Tag(assignmentPresentationIsCorrectTag).Set<std::string>();
...
::mediapipe::Status AnnotationOverlayCalculator::Process(
  ...
  //DrawTextStr(std::string text, int left, int top, int font_size, int thickness, cv::Scalar color)
  const auto &recognizedHandGesture = cc->Inputs().Tag(recognizedHandGestureTag).Get<std::string>();
  renderer_->DrawTextStr(recognizedHandGesture, 275, 50, 35, 1, cv::Scalar(0.0, 0.0, 0.0));

  const auto &assignmentPresentationTime = cc->Inputs().Tag(assignmentPresentationTimeTag).Get<std::string>();
  renderer_->DrawTextStr(assignmentPresentationTime, 400, 30, 15, 1, cv::Scalar(0.0, 0.0, 0.0));

  const auto &assignmentPresentationQuestion = cc->Inputs().Tag(assignmentPresentationQuestionTag).Get<std::string>();
  renderer_->DrawTextStr(assignmentPresentationQuestion, 400, 50, 15, 1, cv::Scalar(0.0, 0.0, 0.0));

  const auto &assignmentPresentationAnswer = cc->Inputs().Tag(assignmentPresentationAnswerTag).Get<std::string>();
  renderer_->DrawTextStr(assignmentPresentationAnswer, 400, 70, 15, 1, cv::Scalar(0.0, 0.0, 0.0));

  const auto &assignmentPresentationIsCorrect = cc->Inputs().Tag(assignmentPresentationIsCorrectTag).Get<std::string>();
  if (assignmentPresentationIsCorrect == "OK"){
    renderer_->DrawCircle(325, 250, 140, 10, cv::Scalar(255.0, 0.0, 0.0));
  }else if (assignmentPresentationIsCorrect == "FINISH"){
    renderer_->DrawTextStr(assignmentPresentationIsCorrect, 140, 300, 75, 3, cv::Scalar(255.0, 0.0, 0.0));
  }
