...
void AnnotationRenderer::DrawTextStr(std::string text, int left, int top, int font_size, int thickness, cv::Scalar color)
{
...
void AnnotationRenderer::DrawCircle(int pointX, int pointY,int radius, int thickness, cv::Scalar color)
{
  cv::Point point_to_draw(pointX, pointY);
  cv::circle(mat_image_, point_to_draw, radius, color, thickness);
}
